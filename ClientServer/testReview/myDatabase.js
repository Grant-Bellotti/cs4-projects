...
myDatabase.prototype.postData = function(_data) {
  ...
  this.data[dataIndex++] =
  new Data(_data.id,_data.name,_data.color,_data.residence,_data.image);
  return true;
}
...
myDatabase.prototype.getData = function(id) {
  for (let i=0;i<this.data.length;i++) {
    ...
    {
      return(new Data(this.data[i].id,this.data[i].name,this.data[i].color,
                      this.data[i].residence,this.data[i].image));
    }
  }
  return null;
}
